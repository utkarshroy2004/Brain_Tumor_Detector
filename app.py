...
def classify_tumor_type(confidence):
    ...
    Note: This is a simplified simulation. In production, train a multi-class CNN model
    with datasets like BraTS (Brain Tumor Segmentation) that include multiple tumor types
    """
    if confidence >= 0.90:
...
# Define the CNN model (same architecture as in notebook)
class CNN(nn.Module):
    def __init__(self):
...
try:
    ...
    print("⚠️ WARNING: brain_tumor_model.pth not found. Please train and save the model first.")
    print(f"Expected location: {model_path}")
    print("Run the notebook and execute the save model cell.")
...
def predict():
    ...
    if file:
        ...
        try:
            ...
            img_tensor, img_rgb = preprocess_image(filepath)
